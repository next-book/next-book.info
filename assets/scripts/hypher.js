...
// Used under BSD license https://github.com/bramstein/hypher/blob/master/BSD.txt
...
window['hyphenate'] = function (els, language) {
    if (window['Hypher']['languages'][language]) {
        return els.forEach(function (el) {
            var i = 0, len = el.childNodes.length;
            for (; i < len; i += 1) {
                if (el.childNodes[i].nodeType === 3) {
                    el.childNodes[i].nodeValue = window['Hypher']['languages'][language].hyphenateText(el.childNodes[i].nodeValue);
                }
            }
        });
    }
